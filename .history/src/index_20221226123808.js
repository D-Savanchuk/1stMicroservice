...
const reqInstance = axios.create({
    ...
    const getLots = async () => {
        ...
        companies.forEach(async (el)=>{            
            const { data } = await reqInstance.get(`https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol=IBM&interval=5min&apikey=A93VLA5M6XJN979E`);
            console.log(data);
            const keys = Object.keys(data['Time Series (5min)']);
            finalArray.push({'Symbol': data['Meta Data']['2. Symbol'],
            'Last': data['Meta Data']['3. Last Refreshed'],f
