...
const urls = [
    `https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol=IBM&interval=5min&apikey=DPBZXCTKST7UZ5BU`
]
    const getLots = async () => {
        try {
          const response = await Promise.all(
            urls.map(url => reqInstance.get(url).then(res => res.json()))
          )
